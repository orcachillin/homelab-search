...
export function ResultCard(result: SearchResult) {
	...
	return <article class="list-group-item search-result-item text-light border-secondary d-flex align-items-center" tabindex="0">
		...
		<div class="flex-grow-1 text-truncate">
			...
			{result.description && <p class="result-details small text-muted mb-0">{result.description}</p>}
		...
		{result.actions.length > 0 && <div class="btn-group btn-group-sm ml-3" role="group">{result.actions.map((action) => <button type="button" class={`btn btn-${action.color}`} hx-post="/api/provider-action" hx-vals={JSON.stringify({ provider: action.provider ?? result.provider.name, action: action.id, resultId: action.resultId ?? result.id })} hx-target="#action-status" hx-swap="innerHTML" hx-indicator="#search-spinner" data-download-action={action.id === "download" ? "true" : undefined}>
